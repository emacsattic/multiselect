...
;; $Id: $
;; Copyright (C) 2006-2009 by Stefan Kamphausen
...
;; concatenation of these or delete the selected regions.

;;; ToDo
;; * Testing during daily usage.
;; * Customize at least the face (well, yes, might be overkill).

;;; Changes
;; Version 1.1
;;    added multiselect-delete-selection
;; Version 1.0
;;    Initial Release

;;; Code:
(defvar multiselect-version "1.1"
  "Version number of multiselect.")
...
(defun multiselect-create-overlay (begin end)
  ...
  (let ((overlay (make-overlay begin end (current-buffer) nil t)))
    ...
    (overlay-put overlay 'multiselect-selection t)
    overlay))
...
(defun multiselect-delete-selection ()
  "Delete all multiselected contents."
  (interactive)
  (while multiselect-overlays
    (let ((this-ol (car multiselect-overlays)))
      (setq multiselect-overlays (cdr multiselect-overlays))
      (delete-region (overlay-start this-ol)
                     (overlay-end   this-ol))
      (delete-overlay this-ol))))
...
Sets C-Y to `multiselect-insert-selection'
  ...
  (global-set-key '[(meta W)]  'multiselect-add-selection-or-line)
